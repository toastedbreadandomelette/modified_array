...
MdStaticArray<_Toutput> MdArrayUtility::reduce(
    ...
    if (axis == -1) {
        ...
        for (size_t index = 0; index < size; ++index) {
            result.__array[0] =
                function_exec(result.__array[0], __values.__array[index], index,
                              index, __values);
        }
        // } else {
        //     std::vector<std::thread> st;
        //     std::vector<_T> __res_total(thread_count, init);
        //     auto _add_int = [&__res_total, &__values, &function_exec](
        //                         const uint8_t thread_number, const size_t
        //                         start, const size_t end) {
        //         for (size_t index = start; index < end; ++index) {
        //             __res_total[thread_number] = function_exec(
        //                 __res_total[thread_number], __values.__array[index],
        //                 index, index, __values);
        //         }
        //     };
        ...
        //     for (auto &result_th : __res_total) {
        //         result.__array[0] = function_exec(result.__array[0],
        //         result_th);
        //     }
        // }
        return result;
                ...
                for (size_t index = thread_number * skip_index;
                    ...
                    for (size_t loop_time = 0, axis_index = 0;
                        ...
                        for (size_t block_index = 0; block_index < skip_index;
                            ...
                            result.__array[index + block_index] = function_exec(
                                result.__array[index + block_index],
                                __values.__array[loop_time + block_index],
                                axis_index, loop_time + block_index, __values);
                        }
                    }
                }
            };
...
MdStaticArray<_Toutput> MdArrayUtility::reduce(
    ...
    const std::function<
        _Toutput(const _Toutput, const _T, const size_t, const size_t,
                 const MdStaticArrayReference<_T> &)> &function_exec,
    const _T init, const int axis) {
    return reduce<_Toutput, _T>(
        MdStaticArray<_T>(*__values.__array_reference, __values.offset,
                          __values.shp_offset),
        function_exec, init, axis);
}
