...
template <typename T>
MdStaticArray<size_t> MdArrayUtility::argmin(const MdStaticArray<T> &values,
                                             const int axis) {
    if (axis == -1) {
...
template <typename T>
MdStaticArray<size_t> MdArrayUtility::argmin(
    const MdStaticArrayReference<T> &values, const int axis) {
    return argmin<T>(MdStaticArray<T>(*values.__array_reference, values.offset,
                                      values.shp_offset),
                     axis);
}
