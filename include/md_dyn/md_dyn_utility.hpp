...
MdDynArray<_T> execute_mapping_fn(MdDynArray<_T> &__values,
...
template <typename _T, typename _func>
_T execute_accumulate_fn(MdDynArray<_T> &__values, const _func &function_exec,
                         const _T init) {
    const size_t size = __values.get_size();
    _T result = init;
    const uint8_t thread_count = ::thread_count;
...
_T execute_accumulate_n_merge_fn(MdDynArray<_T> &__values,
                                 const _func &function_exec,
...
MdDynArray<_T> f_log_10(MdDynArray<_T> &__values) {
    return execute_mapping_fn(__values,
