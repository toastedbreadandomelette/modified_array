...
class MdDynArray {
 public:
    ...
    inline void append(const _T &value) const { __array.push_back(value); }

    inline void emplace(const _T &value) const { __array.emplace_back(value); }
    ...
    MdDynArray<_T2> __add_internal(const MdDynArray<_T1> &__other,
                                   const _T2) const;
    ...
    MdDynArray<bool> __comp_eq_internal(const MdDynArray<_T1> &__other) {
        ...
        if (MdDynArray::thread_count == 1 || __array.size() <= threshold_size) {
            ...
            for (int i = 0; i < thread_but_one; ++i) {
                st.emplace_back(
                    std::thread(_add_int, block * i, block * (i + 1)));
            }

            st.emplace_back(
                std::thread(_add_int, block * thread_but_one, size));
    ...
    inline size_t get_size() const { return __array.size(); }
};
