...
void serial_init(int m, int n, double *a, int lda) {
    int count = 1;
    for (int j = 0; j < n; j++) {
        for (int i = 0; i < m; i++) A(i, j) = count++;
    }
}

void random_init(int m, int n, double *a, int lda) {
    for (int j = 0; j < n; j++) {
        for (int i = 0; i < m; i++) A(i, j) = 2.0 * drand48() - 1.0;
    }
}
...
void AddDot4x4(int k, double *a, int lda, double *b, int ldb, double *c,
    ...
    for (p = 0; p < k; p++) {
        ...
        b_p1_vreg.v =
            _mm_loaddup_pd((double *)(b + 1)); /* load and duplicate */
        b_p2_vreg.v =
            _mm_loaddup_pd((double *)(b + 2)); /* load and duplicate */
        b_p3_vreg.v =
            _mm_loaddup_pd((double *)(b + 3)); /* load and duplicate */
...
void InnerKernel(int m, int n, int k, double *a, int lda, double *b, int ldb,
    ...
    //     packedA[ m * k ];
    //   static double
    //     packedB[ kc*nb ];    /* Note: using a static buffer is not thread
    //     safe... */
...
void matmul(int m, int n, int k, double *a, int lda, double *b, int ldb,
    ...
    */
    ...
    if (a == NULL || b == NULL || c == NULL) {
        printf(
            "Argument Error : One of the input arguments to matmul() was "
            "NULL\n");
        return;
    }
